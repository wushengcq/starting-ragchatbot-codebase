...
class TestRAGSystemIntegration:
    ...
    def test_query_with_content_question(self, populated_rag_system):
        ...
        with patch.object(populated_rag_system.ai_generator, "generate_response") as mock_generate:
            mock_generate.return_value = (
                "Based on the course materials, MCP is a protocol for AI assistants."
            )
    ...
    def test_query_with_outline_question(self, populated_rag_system):
        ...
        with patch.object(populated_rag_system.ai_generator, "generate_response") as mock_generate:
            mock_generate.return_value = (
                "The MCP course has 3 lessons covering introduction, setup, and tools."
            )
    ...
    def test_query_with_session_id(self, populated_rag_system):
        ...
        with patch.object(populated_rag_system.ai_generator, "generate_response") as mock_generate:
            ...
            response, sources = populated_rag_system.query(
                "What is MCP?", session_id="test-session-123"
            )
            ...
            history = populated_rag_system.session_manager.get_conversation_history(
                "test-session-123"
            )
    ...
    def test_vector_store_data_persistence(
    ...
    def test_vector_store_filter_retrieval(self, populated_rag_system, sample_course):
        ...
        results = populated_rag_system.vector_store.search(query="protocol", course_name="MCP")
    ...
    def test_sources_include_urls(self, populated_rag_system):
        ...
        populated_rag_system.tool_manager.execute_tool("search_course_content", query="MCP")
    ...
    def test_query_with_invalid_session_id(self, populated_rag_system):
        ...
        with patch.object(populated_rag_system.ai_generator, "generate_response") as mock_generate:
            ...
            response, sources = populated_rag_system.query(
                "Test", session_id="brand-new-session-xyz"
            )
    ...
    def test_add_single_course_document(self, mock_rag_system, sample_course):
        ...
        with patch.object(
            mock_rag_system.document_processor, "process_course_document"
        ) as mock_process:
